...
int TAT_Dump_tShipCurrentState(void *data, char *dest) {
	...
	strcat(dest,", ");
	TAT_Dump_int(&(((tShipCurrentState *)data)->stepsSlowing),dest);
	strcat(dest,", ");
	TAT_Dump_int(&(((tShipCurrentState *)data)->deceleration),dest);
	strcat(dest,", ");
	TAT_Dump_int(&(((tShipCurrentState *)data)->changeDirection),dest);
	strcat(dest,", ");
	TAT_Dump_tCoordinates(&(((tShipCurrentState *)data)->fightCoordinates),dest);
	strcat(dest," .)");
	return 1;
}
...
int TAT_Free_tShipCurrentState(tShipCurrentState * x) {
	...
	TAT_Free_tCoordinates(&(x->fightCoordinates));
...
tShipCurrentState TAT_Copy_tShipCurrentState(tShipCurrentState src) {
	...
	x.stepsSlowing=src.stepsSlowing;
	x.deceleration=src.deceleration;
	x.changeDirection=src.changeDirection;
	x.fightCoordinates=TAT_Copy_tCoordinates(src.fightCoordinates);
	return x;
}
...
int TAT_Compare_tShipCurrentState(tShipCurrentState x1, tShipCurrentState x2) {
	...
	if(x1.stepsSlowing!=x2.stepsSlowing) return 1;
	if(x1.deceleration!=x2.deceleration) return 1;
	if(x1.changeDirection!=x2.changeDirection) return 1;
	if(TAT_Compare_tCoordinates(x1.fightCoordinates,x2.fightCoordinates)) return 1;
	return 0;
}
...
{"Test8N0", TAT_T_battleshipstest_Test8N0, TAT_TI_battleshipstest_Test8N0}};
